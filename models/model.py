...
class OV9DEncoder(nn.Module):
    def __init__(self, 
                 ...
                 out_dim=1024, ldm_prior=[320, 640, 1280+1280], sd_path=None, text_dim=768, 
                 ):
        super().__init__()
        ...
        if sd_path is None:
            config.model.params.ckpt_path = os.path.join(data_path, 'checkpoints/v1-5-pruned-emaonly.ckpt')
        else:
        ...
        class_embeddings = torch.load(os.path.join(data_path, f'{data_name}_class_embeddings.pth'))
    ...
    def forward(self, x, class_ids=None):
        ...
        # class_embeddings = torch.mean(self.class_embeddings, dim=0, keepdim=True)
